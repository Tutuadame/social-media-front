...
export const SearchBar: React.FC<SearchBarProps> = ({ onSearch, searchExpression, setSearchExpression, resetSearch }) => {  
  ...
  return (
    <>
      ...
      <IconButton ariaLabel="Search" action={() => {onSearch(searchExpression)}} type="button" style={BUTTON.basic}>{searchIconSVG}</IconButton>
      <IconButton action={resetSearch} ariaLabel="Reset" style={BUTTON.basic}> {resetSVG} </IconButton>
    </>
  );
};
