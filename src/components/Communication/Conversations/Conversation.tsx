...
export const ConversationComponent: React.FC<ConversationProps> = ({ conversation, removeConversation, handleSelectConversation }) => {
  ...
  const removeSelf = async () => {
    try {      
      await deleteConversation(conversation.id, accessToken.current);
  ...
  return (
    <div key={conversation.id} onClick={() => {handleSelectConversation(conversation.id)}} className="flex relative flex-row px-2 border-r-2 border-l-2 border-transparent pb-1">
