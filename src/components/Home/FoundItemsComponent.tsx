...
export const FoundItemsComponent: React.FC<FoundItemsComponentProps> = ({ foundProfiles, isSearchOn, onSearch, loadMoreStyle, searchExpression }) => {  
      ...
      :
        <>
          {foundProfiles?.map((profile) => {
            return <ProfileButton profile={profile}/>
          })}
          <LoadMoreButton
            callItems={() => {onSearch(searchExpression)}}
