...
import {ConversationMember, CreateMemberRequest} from "../../interface/communication/member";
...
export const useGetMembers = (members: string[], accessToken: string, options?: CustomQueryOptions<ConversationMember[]>) =>
  useQuery<ConversationMember[]>({
    queryFn: async () => {
      const response = await fetch(getMembersPath, {
        method: POST_METHOD,
